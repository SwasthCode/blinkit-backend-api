  ...
  try {
    ...
    for (
      ...
      try {
        ...
        if (!saved) {
          console.error(
            `Failed to generate unique ID for order ${doc._id} after ${maxAttempts} attempts.`,
          );
          failedCount++;
        }
      } catch (err) {
      ...
      if (processedUrl % batchSize === 0) {
        console.log(
          `Progress: ${processedUrl}/${totalToMigrate} processed. Success: ${successCount}, Failed: ${failedCount}, Skipped: ${skippedCount}`,
        );
      }
    }
