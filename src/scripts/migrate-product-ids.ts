    ...
    try {
        ...
        for (let doc = await cursor.next(); doc != null; doc = await cursor.next()) {
            try {
                ...
                while (!saved && attempts < maxAttempts) {
                    ...
                    try {
                        const result = await productModel.updateOne(
                            { _id: doc._id, $or: [{ product_id: { $exists: false } }, { product_id: null }] },
                        ...
                        if (result.modifiedCount === 1) {
                            ...
                            if (current && current.product_id) {
                                ...
                                console.warn(`Failed to update product ${doc._id}, retrying...`);
                            }
                        }
                    } catch (err) {
                        if (err.code === 11000) { // Duplicate key error
                            console.warn(`Duplicate ID generated ${newProductId}, retrying...`);
                        } else {
                            throw err;
                        }
                    }
                ...
                if (!saved) {
                    console.error(`Failed to generate unique ID for product ${doc._id} after ${maxAttempts} attempts.`);
                    failedCount++;
                }
            ...
            if (processedCount % batchSize === 0) {
                console.log(`Progress: ${processedCount}/${totalToMigrate} processed. Success: ${successCount}, Failed: ${failedCount}, Skipped: ${skippedCount}`);
            }
        }
