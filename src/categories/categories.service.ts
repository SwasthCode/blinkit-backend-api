...
export class CategoriesService extends BaseService<CategoryDocument> {
  ...
  async create(createCategoryDto: any, file?: Express.Multer.File): Promise<CategoryDocument> {
    ...
    if (file) {
      const imageUrl = await this.firebaseService.uploadFile(file, 'categories');
